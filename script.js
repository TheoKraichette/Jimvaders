...
var STEPSPEED = 3; // Multiplicateur
...
function resetGame() {
	let ScoreFin = parseInt($('#score').text());
	document.location.href="level2.html?score=" + ScoreFin;
}
...
function win() {
	...
	LEVEL++;
	$('#filter').show('pulsate').html('<span style="font-size:40px;"><b>LEVEL '+LEVEL+'</b></span><br /><br />Appuyez sur "S" pour commencer');
	resetGame();
...
function loseLife() {
	...
	$('#game').stop().css('background', 'white').animate({backgroundColor: 'black'}, 2000);
...
function gameOver(score) {
	...
	LEVEL = 1;
	$('#filter').show('pulsate').html('<span style="font-size:40px;"><b>GAME OVER</b></span><br />Score : '+score+'<br /><br /><br />Appuyez sur S pour commencer');
	resetGame();
	$('#lifes').html('<img src="heart.png" class="life" /><img src="heart.png" class="life" /><img src="heart.png" class="life" />');
	$('#score').hide().text('0');
}
...
function startLevel(level) {
	...
	var top = 50;
	// -------------------------------------
	// INSERTION DES ENNEMIS SELON LE NIVEAU
	// -------------------------------------
	...
	enemiesShotInterval = setInterval(function() {
		...
		var lowests = []; // ennemis les plus bas / colonnes
		for(var i = 0; i < 8; i++) {
			lowest = -1;
			for(var j = 0; j < 7; j++) {
